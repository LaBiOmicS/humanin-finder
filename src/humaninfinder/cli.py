...
def predict(input, output, threshold, table, hmm, rescue, cpus, all_candidates):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=cpus) as executor:
        ...
        for future in futures:
            ...
            if all_candidates:
                for cand in scored_candidates:
                    click.echo(f"{rec_id:<25} | {cand['status']:<15} | {cand['start']}-{cand['end']:<20} | {cand['score']:.3f}")
                ...
                if final_selection:
                    ...
                    click.echo(f"{rec_id:<25} | {status_str:<15} | {best_cand['start']}-{best_cand['end']:<20} | {best_cand['score']:.3f}")
